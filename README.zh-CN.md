...
点击任务工具栏中的**终端**，直接弹出自动连接的 Bash PTY 窗口，不需要启动按钮或操作工具栏。输出改用事件唤醒的长轮询，支持键盘快捷键、交互程序及自动断线重连。命令以映射的非 root 系统/租户账户直接执行，**不经过 Codex 沙箱审批**。关闭窗口保留会话，输入 `exit`、按 Ctrl+D 或 30 分钟无请求后回收；服务重启不保留 shell 状态。安全边界、限制和部署要求见 [Web 终端](docs/TERMINAL.md)。

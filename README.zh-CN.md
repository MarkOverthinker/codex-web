...
Codex Web 是一个非官方、自托管的 OpenAI Codex CLI 网页工作台。它提供持久化会话、未发送草稿、附件与交付文件、结果文件页内悬浮预览、服务器端任务排队、实时引导、可续接的终止/中断记录、会话归档、完整工作记录、完成任务未读提示、引用提问、自动命名、字号调节以及可选的语音转写。
...
每个任务会维护一个“输出文件”横条，任务生成的 Markdown、纯文本、CSV、图片和 PDF 可以直接在当前任务页的悬浮预览框内查看；文本类预览超过 5 MiB 时会退化为下载，避免大文件占用浏览器内存。

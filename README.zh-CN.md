...
每个任务会维护一个“输出文件”横条，任务生成的 Markdown、纯文本、CSV、图片和 PDF 可以直接在对话窗口左侧展开的并排预览面板中查看；文本类预览超过 5 MiB 时会退化为下载，避免大文件占用浏览器内存。
...
左侧任务列表会按工作目录自动分类：独立工作区、已收藏目录各成一类，未收藏目录统一进入“临时工作区”。你可以创建自定义分类、把某个目录连同其全部任务移入自定义分类、置顶分类并调整置顶顺序；分类定义、目录归属和置顶顺序保存在服务端，展开/折叠状态保存在浏览器。

...
在电脑版网页顶端点击“API 计费统计”（手机版不显示该入口），可查看最近 7/30/90/365 天的输入、输出、缓存 token、缓存命中率和按每百万 token 规则计算的估算费用，并按 API 源和模型拆分。每个模型均可手动设置谷时输入、缓存输入、缓存写入和输出费率，也可启用峰时费率并配置峰时起止时间、星期和 IANA 时区；跨午夜时段同样支持，计费按每次调用的发生时间自动选择峰谷费率。对兼容 New API 或其他 JSON 计费端点的源，可填写计费 JSON 地址（或尝试标准候选地址）后按源同步。不同上游没有统一的计费接口，自动同步只接受能识别出模型及 input/output 每百万 token 价格的 JSON；未配置费率的调用仍保留用量，但不会虚构费用。

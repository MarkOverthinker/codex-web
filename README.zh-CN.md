...
运行中的工作记录不再形成独立的纵向滚动区，而是按照现有记录上限随页面自然展开。排队与运行状态使用不同图标；排队中的任务可以直接跳过队列立即启动，适合确认同目录其他任务不会冲突的场景；任务操作收进稳定的菜单。用户终止任务后，关键执行过程会保留为历史消息；服务意外重启也会明确标记未完成任务，避免把中断误认为完成或自动重复执行。容器正常停止时会等待在途任务结束，已排队任务继续保存在服务器。

...
侧栏 **自动任务** 支持按每日时间和时区自动创建会话、发送固定提示词，可配置运行目录、模型、思考强度及权限模式。专用管理页提供编辑、启停、删除与立即运行；运行记录页保留状态、配置快照和会话入口。停机只补跑一次，上次未结束则跳过重叠触发。详见 [自动任务](docs/AUTOMATIONS.md)。
...
点击任务工具栏中的**终端**，在聊天区右侧展开自动连接的 Bash PTY 侧栏，不使用悬浮窗口或遮罩，不需要启动按钮或操作工具栏。桌面端可调整侧栏宽度，窄屏下改为全宽面板。输出使用事件唤醒的长轮询，支持键盘快捷键、交互程序及自动断线重连。命令以映射的非 root 系统/租户账户直接执行，**不经过 Codex 沙箱审批**。关闭终端栏保留会话，输入 `exit`、按 Ctrl+D 或 30 分钟无请求后回收；服务重启不保留 shell 状态。安全边界、限制和部署要求见 [Web 终端](docs/TERMINAL.md)。

...
export function PresetPromptManagerDialog({ open, onClose, onChanged }: {
  ...
  function beginCreate() {
    setEditing("new"); setName(""); setContent(""); setError(""); setNotice("");
  }

  function beginEdit(preset: PresetPrompt) {
    setEditing(preset); setName(preset.name); setContent(preset.content); setError(""); setNotice("");
  }
  ...
  async function save() {
    ...
    try {
      if (editing === "new") {
        await api.createPresetPrompt(trimmedName, trimmedContent);
        setNotice("预设已添加。");
      } else {
        await api.updatePresetPrompt(editing.id, { name: trimmedName, content: trimmedContent });
        setNotice("预设已更新。");
      }
      await refresh();
      setEditing(null);
  ...
  return createPortal(<div className="preset-manager-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="preset-manager" role="dialog" aria-modal="true" aria-label="预设 Prompt 管理">
      ...
      <p className="preset-manager-hint">预设 Prompt 保存在当前账户下；在对话框下方的预设列表中勾选后，会随每次任务自动附加给 Agent。</p>
      ...
      <footer className="preset-manager-footer"><small>启用数量上限为每个对话 20 条；删除预设会同步从所有对话移除。</small><button type="button" className="preset-manager-close" onClick={onClose}>关闭</button></footer>
    </section>
  </div>, document.body);
}

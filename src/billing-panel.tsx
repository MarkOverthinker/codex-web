...
type Draft = { input: string; cached: string; cacheWrite: string; output: string; currency: string };
...
export function BillingPanel({ open, onClose, providers, builtinModels }: Props) {
  ...
  function updateDraft(model: BillingModel, field: keyof Draft, value: string) {
    const key = `${model.providerId}:${model.modelId}`;
  ...
  async function saveRule(model: BillingModel) {
    ...
    const payload = { inputPerMillion: Number(draft.input), cachedInputPerMillion: Number(draft.cached), cacheWritePerMillion: Number(draft.cacheWrite), outputPerMillion: Number(draft.output), currency: draft.currency };
  ...
  return createPortal(<div className="billing-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="billing-panel" role="dialog" aria-modal="true" aria-label="API 调用计费统计">
      ...
      {!state || loading ? <div className="billing-empty"><LoaderCircle size={20} className="spin" />正在加载统计…</div> : <>
        ...
        <div className="billing-section"><h3>Token 计费规则</h3><p className="billing-hint">单位为每 1,000,000 tokens；缓存输入和缓存写入可分别设置。未配置规则的调用只统计 token，不计入费用。</p><div className="billing-sync"><input value={pricingUrl} onChange={(event) => setPricingUrl(event.target.value)} placeholder="可选：New API 计费 JSON 地址" /><div>{sourceOptions.filter((source) => source.id !== BUILTIN_PROVIDER_ID).map((source) => <button type="button" key={source.id} disabled={Boolean(syncingId)} onClick={() => void syncPricing(source.id)}>{syncingId === source.id ? <LoaderCircle size={14} className="spin" /> : <RefreshCw size={14} />}同步 {source.name}</button>)}</div></div><div className="billing-table-wrap"><table className="billing-table billing-rules-table"><thead><tr><th>源 / 模型</th><th>输入</th><th>缓存输入</th><th>缓存写入</th><th>输出</th><th>货币</th><th /></tr></thead><tbody>{allModels.length === 0 ? <tr><td colSpan={7} className="billing-empty-cell">还没有可配置的模型；调用一次或先导入模型目录。</td></tr> : allModels.map((model) => { const draft = draftForModel(model); const key = `${model.providerId}:${model.modelId}`; return <tr key={key}><td><strong>{model.displayName}</strong><small>{model.providerName} · {model.modelId}</small></td><td><input aria-label={`${model.displayName} 输入费率`} value={draft.input} onChange={(event) => updateDraft(model, "input", event.target.value)} /></td><td><input aria-label={`${model.displayName} 缓存输入费率`} value={draft.cached} onChange={(event) => updateDraft(model, "cached", event.target.value)} /></td><td><input aria-label={`${model.displayName} 缓存写入费率`} value={draft.cacheWrite} onChange={(event) => updateDraft(model, "cacheWrite", event.target.value)} /></td><td><input aria-label={`${model.displayName} 输出费率`} value={draft.output} onChange={(event) => updateDraft(model, "output", event.target.value)} /></td><td><input aria-label={`${model.displayName} 货币`} value={draft.currency} maxLength={3} onChange={(event) => updateDraft(model, "currency", event.target.value.toUpperCase())} /></td><td><button type="button" className="billing-save" disabled={savingKey === key} onClick={() => void saveRule(model)}>{savingKey === key ? <LoaderCircle size={14} className="spin" /> : <Save size={14} />}保存</button></td></tr>; })}</tbody></table></div></div>
      </>}
    </section>
  </div>, document.body);
}

...
[Product overview](../README.md) · [Documentation index](README.md) · [中文开发指南](DEVELOPMENT.zh-CN.md)

This guide retains the detailed README: setup, architecture, operational behavior and development notes. Commands and inline repository paths are relative to the repository root, unless stated otherwise. Android version notes describe individual previews; use the Android documentation and release records for their status.

An independent Kotlin/Jetpack Compose Android preview client is available in `android/`. The 0.4.2 preview replaces the floating bottom bar with three icon-only tabs in a compact 48dp row, preserving accessible names, selection, drafts and reading position. It builds on a chat-first home, collapsible project task drawer, inline queue entry, encrypted recent-view caching and explicit local draft-save outcomes. It reuses the existing API, server-side drafts, queue and event stream, without a WebView shell. The mobile web UI remains available separately. This is not an official OpenAI app or a fully acceptance-tested commercial release. See [Android setup, feature mapping, and remaining gaps](ANDROID.md) (Chinese).

**Current Android release:** `0.5.1-preview` (versionCode 13) is published on the maintained download page. It adds direct horizontal swipe paging across the three root screens (Chat, Workspace, Profile) with the bottom navigation animating through the same pager state, on top of the unified native theme, safer welcome suggestions, clearer composer states and reading behavior, a task-focused workspace, and local Review filters. Install `codex-native-android-0.5.1-preview.apk`; an existing 0.5.0-preview installation signed with the same key can be upgraded in place without uninstalling. See the [release and acceptance record](../releases/android/0.5.1-preview.md) and [iteration plans, PRDs and QA](../tasks/prd-android-05.md).
...
- Daily automations with a working directory, explicit time zone, model, reasoning effort, permission mode, and fixed prompt; dedicated management and run-history views create a fresh durable conversation for every run. See [Automations](AUTOMATIONS.md) for catch-up, overlap, and deployment rules.

- A unified repository workspace with branch/upstream status, change counts, a filterable file tree, syntax-highlighted diffs, rendered Markdown, and source previews. Compare working-tree, staged, or branch changes in a resizable desktop pane or mobile layout. Commit/push requests require explicit confirmation and run as durable Codex tasks under existing identity, sandbox, and approval rules—not as direct Git write endpoints. See [Repository workspace](GIT_REVIEW.md) for behavior and limits.
...
- Optional multi-provider management with a "source · model" picker: native Responses providers remain direct, while Chat Completions-only providers can run through a per-task bundled `codex-relay`; management is disabled by default per user and supports source/model visibility plus task-level switching through app-server `modelProvider` (see [docs/PROVIDER_MANAGEMENT.md](PROVIDER_MANAGEMENT.md))
- API usage and billing analytics from live Web turns plus deduplicated CLI/Desktop/VS Code rollout backfills, grouped by provider, model, and client, with input/output/cache token totals, cache-hit rate, configurable valley/peak per-million-token rules, rate history, and a full-history reprice action
...
For the public build, the web process has no Docker socket, host filesystem mount, or root bridge. See [Architecture](ARCHITECTURE.md) and [Security](SECURITY.md) before adapting the extension pattern to your own environment.
...
normal login. See [Host mode](DEPLOYMENT.md#host-mode-machine-users-as-tenants)
...
The container binds to loopback by default. Proxy `/codex-web/` to `http://127.0.0.1:37821/codex-web/` and preserve WebSocket/SSE-friendly buffering settings. See [Deployment](DEPLOYMENT.md).

## Usage and billing

The Web billing panel also supports **all history** and **custom date/time ranges**. Start/end dates use the browser's local timezone; the end minute is included (default 00:00–23:59 includes the entire selected day). The same range applies to totals and provider/model breakdowns and remains selected after saving or syncing rates. HTTP clients can send ISO timestamps as `from`/`to` (inclusive start, exclusive end) to billing endpoints, or `days=0` for all history. Invalid ranges are rejected before any pricing mutation.

The collapsible pricing editor defaults to models that are visible within enabled providers, plus available built-in models. **Show inactive models** reveals saved rates for hidden, disabled, or removed models. This changes only editor visibility: historical calls, rate versions, and costs remain intact. Model availability is still managed under API sources and models; force reprice continues to affect all history, not just the selected range.

On desktop, click **API usage and billing** in the top bar (on mobile, open **Conversation tools → API statistics**, or use account settings) to review 7/30/90/365-day input, output, and cache token totals, cache-hit rate, and estimated cost grouped by provider, model, and Codex client. Web turns are recorded from live app-server events. Every minute, a background reconciler incrementally scans Codex rollouts, deduplicates by thread/turn, and backfills missing CLI, Exec, VS Code, Desktop, or interrupted usage; **sync local usage** runs it immediately. Host mode scans both the system user's `~/.codex` and the private Web Codex Home. Prices are configured per million tokens for uncached input, cache reads, cache writes, and output. The usage event reports total input tokens, so uncached input is calculated as `max(input_tokens - cached_input_tokens - cache_write_input_tokens, 0)` before applying the input price. The regular values are used as valley prices by default; each model can optionally define peak prices, a daily peak window (including cross-midnight windows), applicable weekdays, and an IANA timezone. When a price changes, the prior rule remains effective for usage before the change and the new rule is used afterward. The **force reprice** action clears those rule versions and applies the current rule to all historical usage, which is useful for correcting an initially wrong price. Opening the panel only reads local pricing rules and does not request remote configuration; click **sync remote pricing** to explicitly try compatible pricing endpoints for every enabled provider, prioritizing New API `/api/pricing` and `/api/ratio_config`. Because upstreams do not share one pricing API, sync only imports responses that expose a model plus input/output token prices. Calls without a configured rule remain visible but are deliberately excluded from estimated cost.
...
Codex runs as a dedicated non-root Unix user. The web process can coordinate that worker through a local supervisor but the public edition has no Docker socket, no host filesystem bridge, and no host-root execution path. Review [Security](SECURITY.md) before exposing an instance to the internet.
...
See [Offline upgrade guide](OFFLINE_UPGRADE.md) for packaging, in-place upgrades, full backups, and deployment boundaries.

## Interactive terminal

Click **Terminal** in the task toolbar to open an automatically connected Bash PTY sidebar beside the chat workspace in the task directory, without an extra start button or command toolbar. The desktop sidebar is resizable; on narrow screens it becomes a full-width panel. Output arrives through event-driven long polling instead of periodic polling; use keyboard shortcuts and interactive programs directly. Commands execute directly as the mapped non-root system/tenant account, **without Codex sandbox approval**. Closing the sidebar preserves the session; `exit`/Ctrl+D or 30 minutes without requests ends it. See [Web terminal](TERMINAL.md) for security boundaries, limits and deployment requirements.

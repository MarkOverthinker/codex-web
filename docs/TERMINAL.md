...
Open a task and choose **Terminal** in the task toolbar (inside **Conversation tools** on mobile). A terminal sidebar opens beside the chat workspace and connects automatically, focusing the input when ready. On desktop it participates in the page layout, keeps chat and composer usable, and can be resized from its left edge; the saved width is restored in the same browser. Below 1100 px it overlays the right side, and below 700 px it fills the available viewport. There is no modal, backdrop, separate start button or command toolbar. The mobile back action closes the sidebar. Commands start in the task's selected host working directory, or its conversation workspace in tenant mode.

This is an interactive Bash PTY, not a Codex command proposal. It supports ANSI output, Unicode, shell history within the live session, Tab completion, Ctrl+C, interactive programs and terminal resizing. Use keyboard shortcuts directly: Ctrl+C interrupts, Tab completes, Ctrl+L clears and `exit` or Ctrl+D ends the shell. Escape is forwarded to terminal programs rather than closing the sidebar. Bash starts without loading profile/rc files; source trusted configuration explicitly if required. Automatic Bash history-file persistence is disabled with `HISTFILE=/dev/null`. The initial PATH is `/usr/local/bin:/usr/bin:/bin`; user-specific runtime managers are not automatically loaded.

Closing the sidebar or refreshing the page leaves the shell alive. Reopening automatically attaches to the same running shell and replays its bounded output. Transient connection failures trigger up to three automatic retries; the existing cursor is preserved while reconnecting to the same shell, and uncertain/unsent input is not replayed. Persistent errors remain visible; close and reopen the sidebar to retry. End the shell with `exit` or Ctrl+D. Archiving or deleting a task also closes its terminals before completing the operation. Exiting the shell allows a new shell to be started. Changing the task's working-directory setting does not move an already running shell; exit the shell and reopen the sidebar.
...
- `npx playwright test tests/browser/terminal.spec.ts`: desktop/mobile automatic sidebar connection, absence of command buttons, focused keyboard input, terminal and sidebar resizing, close/reopen, desktop chat coexistence, responsive full-width layout, mobile back and automatic reconnect without duplicated output or input, using mocked transport.

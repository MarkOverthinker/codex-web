...
[产品首页](../README.zh-CN.md) · [文档导航](README.md) · [English development guide](DEVELOPMENT.md)

这里保留原 README 中的完整使用、部署、架构与开发说明。除非另有说明，命令和行内仓库路径均相对于仓库根目录。Android 版本段落记录各次预览迭代，发布状态与限制请结合 Android 文档及发布记录阅读。

侧栏 **自动任务** 支持按每日时间和时区自动创建会话、发送固定提示词，可配置运行目录、模型、思考强度及权限模式。专用管理页提供编辑、启停、删除与立即运行；运行记录页保留状态、配置快照和会话入口。停机只补跑一次，上次未结束则跳过重叠触发。详见 [自动任务](AUTOMATIONS.md)。
...
会话工具中的 **变更**、**环境信息** 和 **文件** 整合为统一仓库工作区：显示当前分支、上游与增删统计，提供可筛选目录树、语法高亮 diff、Markdown 渲染和源码预览；支持工作区、暂存区与分支比较，桌面并排/展开和移动端适配。提交与推送需确认范围后交给当前 Codex 任务执行，沿用账户隔离、持久化队列及审批，不直接绕过执行器修改 Git。使用方式与限制见 [仓库工作区](GIT_REVIEW.md)。
...
公开版 Web 进程没有 Docker socket、宿主文件系统挂载或 root bridge。准备根据扩展架构搭建自己的管理员与远端执行能力前，请先阅读[架构说明](ARCHITECTURE.md)和[安全说明](SECURITY.md)。
...
本地模式：部署独立 CPU 语音服务并设置 `TRANSCRIPTION_PROVIDER=local`。主输入框及侧边聊天支持使用时选择 **SenseVoiceSmall INT8** 或 **Qwen3-ASR-0.6B**；当前浏览器按用户名记住上次选择。桌面语音选择框与模型、思考等级同排；App将选择框收进“任务选项”，输入栏仅保留发送旁的一个语音按钮。再次点击语音按钮仅转写回填，录音中点击发送则识别成功后合并最新草稿并发送一次；失败、空结果或取消不发送。模型准备、离线迁移、服务管理和限制见 [本地语音部署](LOCAL_VOICE.md)。本地服务只接收录音和模型选择，不接收草稿、附件或对话内容，也不会失败后改用云 API。
...
更多信息请参阅 [部署说明](DEPLOYMENT.md)、[架构说明](ARCHITECTURE.md) 与 [安全说明](SECURITY.md)。

## 交互式终端

点击任务工具栏中的**终端**，在聊天区右侧展开自动连接的 Bash PTY 侧栏，不使用悬浮窗口或遮罩，不需要启动按钮或操作工具栏。桌面端可调整侧栏宽度，窄屏下改为全宽面板。输出使用事件唤醒的长轮询，支持键盘快捷键、交互程序及自动断线重连。命令以映射的非 root 系统/租户账户直接执行，**不经过 Codex 沙箱审批**。关闭终端栏保留会话，输入 `exit`、按 Ctrl+D 或 30 分钟无请求后回收；服务重启不保留 shell 状态。安全边界、限制和部署要求见 [Web 终端](TERMINAL.md)。

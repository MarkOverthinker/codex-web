...
At viewport widths up to 900 CSS pixels, the same React workspace uses a single mobile header, on-demand tool sheets, full-screen auxiliary panels, and a composer sized to the visual viewport. `mobile-layout.tsx` owns responsive presentation, focus handling, and the UI-only back callback; `mobile-input.ts` keeps touch-screen Enter behavior consistent across primary and side chats. Business callbacks, authentication, and persistence are shared with the desktop UI, rather than reimplemented in a second client. Browser regression tests use isolated mock APIs and never execute production tasks.

The separate Android client (`android/`, version 0.2+) replaces the former WebView Activity with Kotlin/Compose screens. It implements its own presentation and navigation but reuses the existing HTTP/multipart API, Cookie + CSRF authentication, server drafts, queue, job events and Codex threads. `ClientModel` serializes draft saves, reports saving/success/failure accurately across best-effort logout and session expiry, fences stale session generations, and guards ambiguous send results with one immutable message snapshot before reconciliation; `NativeApi` restricts credentials to the configured HTTPS origin, refuses redirects, never automatically retries mutations, and rejects cookie responses from requests that predate credential clearing. Authentication-scoped Compose state disposes tool sheets, editors and confirmation callbacks when the session ends. Android Keystore protects locally backed-up drafts and session cookies, with drafts scoped by server, username and conversation. SSE cursors and foreground reconciliation preserve the server as the authoritative task state. There is no native task executor, backend schema fork, or change to worker UID isolation. See `docs/ANDROID.md` for the feature matrix and explicit preview gaps; JVM, Compose mock-API and Express/SQLite protocol tests cover different layers rather than constituting production end-to-end acceptance.

Android 0.3 adds a chat-first shell with a swipe-out project/status drawer, bottom navigation and a queue bottom sheet opened above the composer. `TaskGroups` follows web category identifiers and ordering; the additive `latest_job_status` query field distinguishes actual completion from idle state without a schema migration. `SnapshotCache` stores account/origin-scoped encrypted recent views (16 entries, 512 KiB per entry, 4 MiB per account, seven-day TTL), separately from unsent drafts. Cached conversations render before refresh and cannot send until verified; explicit access denial removes the snapshot. Cold starts still validate the server session, so this is not a full offline client. Request indicators belong to the relevant voice, upload, send or refresh control rather than a global page progress bar.
...
Each app-server turn client scopes notifications carrying `threadId` to the thread returned by start, resume, or fork. Child-agent notifications cannot replace the active parent turn, answer, reasoning, or context usage, or complete/fail/cancel the parent job. Parent-thread collaboration items still provide the durable subagent journal. Notifications without a thread ID retain legacy handling; after the parent reaches a terminal state, further notifications are ignored. Regression tests interleave parent and child events through a fake stdio app-server without invoking real agents.

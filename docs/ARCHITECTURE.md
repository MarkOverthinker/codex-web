...
The separate Android client (`android/`, version 0.2+) replaces the former WebView Activity with Kotlin/Compose screens. It implements its own presentation and navigation but reuses the existing HTTP/multipart API, Cookie + CSRF authentication, server drafts, queue, job events and Codex threads. `ClientModel` serializes draft saves, fences stale session generations, retains local drafts across best-effort logout, and guards ambiguous send results with one immutable message snapshot; `NativeApi` restricts credentials to the configured HTTPS origin, refuses redirects and never automatically retries mutations. Android Keystore protects locally backed-up drafts and session cookies, with drafts scoped by server, username and conversation. SSE cursors and foreground reconciliation preserve the server as the authoritative task state. There is no native task executor, backend schema fork, or change to worker UID isolation. See `docs/ANDROID.md` for the feature matrix and explicit preview gaps; JVM, Compose mock-API and Express/SQLite protocol tests cover different layers rather than constituting production end-to-end acceptance.

...
An Android preview client and mobile-first interaction layer are available in this repository. The hybrid client reuses the existing server and business components, with on-demand tool sheets, full-screen panels, and Android file, microphone, download, and back-navigation integration. It is not a native UI rewrite or an official OpenAI app. See [Android setup, feature mapping, and limitations](docs/ANDROID.md) (Chinese).

...
Click **Terminal** in the task toolbar to open an automatically connected Bash PTY window in the task directory, without an extra start button or command toolbar. Output arrives through event-driven long polling instead of periodic polling; use keyboard shortcuts and interactive programs directly. Commands execute directly as the mapped non-root system/tenant account, **without Codex sandbox approval**. Closing the window preserves the session; `exit`/Ctrl+D or 30 minutes without requests ends it. See [Web terminal](docs/TERMINAL.md) for security boundaries, limits and deployment requirements.

...
An independent Kotlin/Jetpack Compose Android preview client is available in `android/`. Version 0.3 builds on the native client with a chat-first home, swipe-out project task drawer, bottom navigation, inline queue entry and encrypted recent-view caching. It reuses the existing API, server-side drafts, queue and event stream, without a WebView shell. The mobile web UI remains available separately. This is not an official OpenAI app or a fully acceptance-tested commercial release. See [Android setup, feature mapping, and remaining gaps](docs/ANDROID.md) (Chinese).

...
An independent Kotlin/Jetpack Compose Android preview client is available in `android/`. Version 0.2 replaces the WebView shell with native chat, task and tool screens backed by the existing API, server-side drafts, queue and event stream. The mobile web UI remains available separately. This is not an official OpenAI app or a fully acceptance-tested commercial release. See [Android setup, feature mapping, and remaining gaps](docs/ANDROID.md) (Chinese).

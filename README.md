...
- A persistent side chat per primary conversation, with an independently resizable desktop pane, a one-click full primary-context snapshot, an independent Codex thread, per-side model/reasoning selection, and one-click source quotes resolved to rollout JSONL path, line, byte offset, JSON Pointer, item ID, and character range

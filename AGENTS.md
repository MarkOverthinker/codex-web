...
- Preserve Git author and committer identities; privacy cleanup targets machine addresses, private domains, deployment paths and credentials, not contributor attribution. Keep the privacy hooks enabled and run `npm run check:privacy` before publishing. Store deployment-specific forbidden terms only in the ignored `.privacy.local.json`, never in source, tests, or commit messages. See `docs/PUBLIC_PRIVACY.md` when needed.
